...
description: Build ROS packages with automatic dependency mapping and support for ROS1 and ROS2
---
...
<ParamField path="distro" type="string" default="Auto-detected">
  ...
  Uses the [automated detection](#automatic-distro-detection) based on the `channels` in the workspace if not specified.
...
<Steps>
  <Step title="Package Detection">
    Parses `package.xml` to determine build type (`ament_cmake`, `ament_python`, `catkin`)
  </Step>
  
  <Step title="Dependency Resolution">
    Maps ROS dependencies to conda packages using RoboStack mappings
  </Step>
  
  <Step title="Environment Setup">
    Configures ROS-specific environment variables
  </Step>
  
  <Step title="Build Execution">
    Uses the appropriate build template based on package type
  </Step>
  
  <Step title="Installation">
    Installs built artifacts to the conda package prefix
  </Step>
</Steps>
